...
export function assembleContext(params: ContextParams): ContextAssemblyResult {
	...
	let _enrichmentTiers: TieredEnrichment | undefined; // Will be consumed in Phase 5 budget pressure tier-aware degradation
...
Original output was too large for the context window. If you need the full content, use: query "SELECT substr(content, 1, 2000) FROM messages WHERE id = '${msg.id}'"`;
	...
	if (!noHistory) {
		...
		_enrichmentTiers = enrichmentTiersL1;
	...
	if (noHistory) {
		...
		_enrichmentTiers = enrichmentTiersL2;
	...
	if (enrichmentBaseline !== undefined && enrichmentMessageIndex >= 0) {
		...
		if (headroom < 2000) {
			...
			const {
				...
				tiers: enrichmentTiersL3,
			} = buildVolatileEnrichment(db, enrichmentBaseline, 3, 3);
			_enrichmentTiers = enrichmentTiersL3;
