...
describe("Context Assembly Pipeline", () => {
	...
	it("should not orphan extra tool_results when a non-tool message appears between them", async () => {
		// Reproduces the exact production failure: 4-tool response where tools complete at
		// slightly different timestamps, causing an assistant text message to land between
		// tool_results in the created_at ordering.
		const testThreadId = randomUUID();
		const testUserId = randomUUID();
		...
		const t0 = "2026-03-27T00:34:49.679Z";
		const t1 = "2026-03-27T00:34:49.680Z"; // last tool result 1ms later
		...
		// Insertion order matches agent loop: assistant text, tool_call, then tool_results
		db.run(
			"INSERT INTO messages (id, thread_id, role, content, model_id, tool_name, created_at, modified_at, host_origin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			[randomUUID(), testThreadId, "assistant", "Let me check...", null, null, t0, t0, "local"],
		);
		db.run(
			"INSERT INTO messages (id, thread_id, role, content, model_id, tool_name, created_at, modified_at, host_origin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			[randomUUID(), testThreadId, "tool_call", toolCallContent, null, null, t0, t0, "local"],
		);
		db.run(
			"INSERT INTO messages (id, thread_id, role, content, model_id, tool_name, created_at, modified_at, host_origin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			[randomUUID(), testThreadId, "tool_result", "pong", null, "tu-1", t0, t0, "local"],
		);
		db.run(
			"INSERT INTO messages (id, thread_id, role, content, model_id, tool_name, created_at, modified_at, host_origin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			[randomUUID(), testThreadId, "tool_result", "host: local", null, "tu-2", t0, t0, "local"],
		);
		// Last tool result arrives 1ms later — appears after the assistant text in time order
		db.run(
			"INSERT INTO messages (id, thread_id, role, content, model_id, tool_name, created_at, modified_at, host_origin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			[randomUUID(), testThreadId, "tool_result", "Available commands: ...", null, "tu-3", t1, t1, "local"],
		);

		const messages = assembleContext({ db, threadId: testThreadId, userId: testUserId });

		// Exactly ONE tool_call — no synthetic extras
		const toolCalls = messages.filter((m) => m.role === "tool_call");
		expect(toolCalls).toHaveLength(1);

		// All three tool_results present and consecutive
		const toolResults = messages.filter((m) => m.role === "tool_result");
		expect(toolResults).toHaveLength(3);

		const firstResultIdx = messages.findIndex((m) => m.role === "tool_result");
		const between = messages
			.slice(firstResultIdx, firstResultIdx + 3)
			.filter((m) => m.role !== "tool_result");
		expect(between).toHaveLength(0);
	});

	it("should not orphan extra tool_results in a multi-tool response", async () => {
		const testThreadId = randomUUID();
		const testUserId = randomUUID();
		...
		// One tool_call with 3 tool_use blocks, followed by 3 separate tool_result messages
		db.run(
			"INSERT INTO messages (id, thread_id, role, content, model_id, tool_name, created_at, modified_at, host_origin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			[randomUUID(), testThreadId, "tool_call", toolCallContent, null, null, t0, t0, "local"],
		);
		...
		db.run(
			"INSERT INTO messages (id, thread_id, role, content, model_id, tool_name, created_at, modified_at, host_origin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			[randomUUID(), testThreadId, "tool_result", "cmd list", null, "tu-2", t0, t0, "local"],
		);
		db.run(
			"INSERT INTO messages (id, thread_id, role, content, model_id, tool_name, created_at, modified_at, host_origin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			[randomUUID(), testThreadId, "tool_result", "host: local", null, "tu-3", t0, t0, "local"],
		);

		const messages = assembleContext({ db, threadId: testThreadId, userId: testUserId });

		// There must be exactly ONE tool_call in the output (no synthetic extras)
		const toolCalls = messages.filter((m) => m.role === "tool_call");
		expect(toolCalls).toHaveLength(1);

		// All three tool_results must be present and consecutive (no non-tool_result between them)
		const toolResults = messages.filter((m) => m.role === "tool_result");
		expect(toolResults).toHaveLength(3);
