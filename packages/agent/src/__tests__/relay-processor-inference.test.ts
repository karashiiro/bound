...
describe("RelayProcessor - executeInference", () => {
	...
	it("emits sync:trigger after each flush to wake the sync loop", async () => {
		// Regression: Before fix, executeInference wrote stream chunks to relay_outbox
		// but never emitted sync:trigger. Without this, chunks sat in the outbox for
		// up to 60s (the sync interval) before being delivered to the requester,
		// causing inference relay timeouts on the hub.
		const mockBackend = new MockBackend();
		mockBackend.pushResponse(async function* () {
			// Produce enough data for at least one intermediate flush (>4KB)
			yield { type: "text" as const, content: "x".repeat(5000) };
		...
		const { eventBus, emitted } = createTrackedEventBus();
		...
		// sync:trigger must have been emitted at least once (once per flush)
		const syncTriggers = emitted.filter((e) => e.event === "sync:trigger");
		expect(syncTriggers.length).toBeGreaterThan(0);

		// Every trigger should have the inference-stream-flush reason
		for (const trigger of syncTriggers) {
			expect((trigger.args as { reason: string }).reason).toBe("inference-stream-flush");
		}

		// Number of triggers should match number of flushes (chunks + stream_end)
		const totalFlushes = (
			db.query("SELECT COUNT(*) as n FROM relay_outbox WHERE stream_id = ?").get(streamId) as {
				n: number;
			}
		).n;
